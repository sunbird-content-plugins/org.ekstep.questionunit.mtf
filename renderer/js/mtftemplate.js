    ...
    <div class="lhs-rhs-container lhs-container">\
      <% _.each(question.data.option.optionsLHS,function(val,key){ %>\
        <div class="lhs-rhs-block lhs-block">\
            ...
            <span <% if(val.image){ %> onclick="org.ekstep.questionunit.baseComponent.showImageModel(event, \'<%= MTFController.pluginInstance.getAssetUrl(val.image) %>\')" <% } %> class="<% if(val.text.length <= 15) { %> short-text <% } else { %> long-text <% }  %>" ><%= val.text %></span>\
            <% if(val.image){ %> \
